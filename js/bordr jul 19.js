...
// 	navigator.geolocation.getCurrentPosition(handleGetCurrentPosition, onError);			
  ...
  function uploadContent(name) {
	...
	console.log(name + ibrdFrom + ', ' + ibrdTo + ', ' + iloc_visi + ', ' + iloc_impo + ', ' + iloc_arti + ', ' + iloc_posi + ', ' + iloc_tim + ', ' + iloc_mon + 
	', ' + iloc_ris + ', ' + ibrdDesc + ', ' + iloc_latlng);
	...
	$.ajax({
		...
		success: function(response){
			...
			if(responseType=="success"){
					  ...
					  goToCrossings();	// go to crossings page
 ...
 function renderMyCrossings(cat,depart) {
	  ...
	  var iuser = $.cookie('user');
		...
		$.ajax({
			...
			data: {device: iuser, cat: cat, depart: depart},
			dataType: "json",
			success: function (data) {
				...
				$(data.crossings).each(function(i, el){
					var fdata;
						...
						fdata = "<article class='box'>" +
								"	<h2>I crossed the "+data.crossings[i].border+" border</h2>";
						if (data.crossings[i].dep!=null) {
						...
						fdata = fdata + "	<div class='row' >" +
								...
								"			<a href='http://db.bordr.org/c/"+data.crossings[i].chash+"'><img src='"+data.crossings[i].img+"' class='img-rounded img-responsive' /></a>" +
								...
								"				</div>" +							
								"				<div class='col-sm-6 col-md-6'>" +
								"					<h5>Where border was crossed</h5>"+
...
//  RELATED BY INDIVIDUAL METRIC							
				fdata =	fdata + "			<div class='row'>" +
								"				<div class='col-md-12'>" +
								"					<h5>Crossing " + data.crossings[i].border + " is like crossing . . .</h5>" +
								"				</div>";
							$(data.crossings[i].related).each(function(ir, elr){
				fdata = fdata + "				<div class='col-xs-4 col-md-4'>" +
								"					<p><b><a href='http://db.bordr.org/border/" + data.crossings[i].related[ir].slug + "/'>" + data.crossings[i].related[ir].border+"</a></b>. They are both <b>"+ data.crossings[i].related[ir].metval + " "+ data.crossings[i].related[ir].metric + "</b>.</p><p><a href='http://db.bordr.org/border/" + data.crossings[i].related[ir].slug + "/'><img src='"+data.crossings[i].related[ir].img+"' class='img-responsive'/ style='max-height:160px;'></a>" +
								"				</div>";
							});
 ...
 function goToCrossings() {
      location.href = '/crossing/';
 }
	  ...
	  var iuser = $.cookie('user');
		...
		$.ajax({
			...
			success: function (data) {
				...
				statusd = ''+
					'<div class="row"><div class="col-sm-12"><h2>I crossed the '+data[0]['border']+' border</h2></div></div>' +	
					'<div class="row">' +
					'	<div class="col-sm-5">' + 
					'		<div class="row">' + 
					'			<div class="col-sm-12">' +
					...
					'			</div>' +
					'			<div class="col-sm-6">' +
					'			<h5 class="where">Where the border was crossed</h5>' +
