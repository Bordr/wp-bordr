...
(function($) {
	$.fn.linkPreview = function(options) {
		...
		$('#postPreview').click(function() {
			...
			description = $('#previewSpanDescription').html();

			if (((trim(text) !== "") || (trim(text) === "" && trim(hrefUrl) !== "")) && (allowPosting === true && isCrawling === false)) {
				$.get('/incl/highlightUrls.php', {
					text : text,
					description : description,
					url : hrefUrl,
					crossing : $.cookie('currentcrossing')
				}, function(urls) {

// 					content = '<div class="previewPosted">' + '<div class="previewTextPosted">' + urls.urls + '</div><a href="' + hrefUrl + '" class="embed"></a></div>';
